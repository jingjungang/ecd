...
public class BackToWork_Frgment extends Fragment implements OnClickListener {
	...
	public View onCreateView(LayoutInflater paramLayoutInflater,
		...
		this.root = paramLayoutInflater.inflate(R.layout.fragment_backtowork,
	...
	private void setDataByPost(int i) {
		...
		if (i == 1) {
			this.rThread = new RequestThread(localArrayList, "http", "post",
					Constant.DEAD_URL, fetchHandler);
		} else if (i == 2) {
			...
			localArrayList.add(new BasicNameValuePair("time", mdate.getText()
					.toString()));
			this.rThread = new RequestThread(localArrayList, "http", "post",
					Constant.DEAD_EDIT_URL, mEidHandler);
		}
		this.rThread.start();
	}
