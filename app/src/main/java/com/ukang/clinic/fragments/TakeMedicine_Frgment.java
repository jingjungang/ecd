...
public class TakeMedicine_Frgment extends Fragment implements OnClickListener {
	...
	public View onCreateView(LayoutInflater paramLayoutInflater,
		...
		this.root = paramLayoutInflater.inflate(
				R.layout.fragement_takemedicine, paramViewGroup, false);
		...
		radioGroup.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(RadioGroup Group, int checkedId) {
				...
				case R.id.n:
					...
					Toast.makeText(getActivity(), "请填写说明", Toast.LENGTH_SHORT)
							.show();
					break;
				}
			}
	...
	private void setDataByPost(int i) {
		...
		if (i == 1) {
			this.rThread = new RequestThread(localArrayList, "http", "post",
					Constant.TAKE_MEDICINE_URL, fetchHandler);
		} else if (i == 2) {
			localArrayList.add(new BasicNameValuePair("no",
					rb_y.isChecked() ? "1" : "0")); // 0 否 1是
			localArrayList.add(new BasicNameValuePair("text", reson.getText()
					.toString()));
			this.rThread = new RequestThread(localArrayList, "http", "post",
					Constant.TAKE_MEDICINE_EDIT_URL, mEidHandler);
		}
		this.rThread.start();
	}
